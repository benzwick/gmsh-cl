...
(defmacro with-recording ((filename) &body body)
  "Record all gmsh API calls made during BODY to FILENAME.
   Appends to the file if it already exists, writes a header if creating new."
  (let ((fn (gensym "FILENAME"))
        (new-p (gensym "NEW-P")))
    `(let* ((,fn ,filename)
            (,new-p (not (probe-file ,fn))))
       (with-open-file (*recording-stream* ,fn
                                           :direction :output
                                           :if-exists :append
